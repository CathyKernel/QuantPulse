...
(function () {
  ...
  BarChartH.prototype.draw = function (hoverOnly) {
    ...
    if (!n) return;
    var f = this.frame();
    var labelW = this.opts.labelW || Math.min(64, Math.max(38, f.w * 0.09));
    ...
    for (i = 0; i < n; i++) {
      ...
      var lbl = String(it.label != null ? it.label : "");
      if (ctx.measureText(lbl).width > labelW - 10) {          // keep long labels inside the band
        while (lbl.length > 1 && ctx.measureText(lbl + "…").width > labelW - 10) lbl = lbl.slice(0, -1);
        lbl += "…";
      }
      ctx.fillText(lbl, f2.x - 8, y + barH / 2);
